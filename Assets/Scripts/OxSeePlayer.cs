...
public class OxSeePlayer : MonoBehaviour
{
    [SerializeField] private float detectionDistance;
    [SerializeField] private OxMovement oxMovement;
    void Start()
    {
        if (oxMovement == null)
        {
            oxMovement = GetComponent<OxMovement>();
        }
    }
    void Update()
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            if (Physics.SphereCast(transform.position, 0.2f, direction, out hit, detectionDistance))
            {
                if (hit.collider.tag == "Player")
                    oxMovement.DetectPlayer(hit.collider.gameObject);
